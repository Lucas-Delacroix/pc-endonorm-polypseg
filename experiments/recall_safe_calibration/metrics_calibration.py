...
def sigmoid_temperature(logits: np.ndarray, temperature: float = 1.0) -> np.ndarray:
    if temperature <= 0:
...
def confusion_counts(prediction: np.ndarray, target: np.ndarray) -> dict[str, float]:
...
def metrics_from_counts(counts: dict[str, float]) -> dict[str, float]:
    ...
    return {
        ...
        "f1": float(f1),
        "specificity": float((tn + SMOOTH) / (tn + fp + SMOOTH)),
    }
...
def calibration_bins(
    ...
    for index in range(n_bins):
        ...
        ece += (count / total) * gap
        bins.append(CalibrationBin(lower, upper, count, confidence, empirical_rate, gap))
    return float(ece), bins


def foreground_region(targets: np.ndarray, dilation_iterations: int = 0) -> np.ndarray:
...
def boundary_metrics_single(prediction: np.ndarray, target: np.ndarray) -> dict[str, float]:
...
def mean_boundary_metrics(predictions: np.ndarray, targets: np.ndarray) -> dict[str, float]:
...
def compute_metrics(
    ...
    *,
    ...
    metrics.update({
        ...
        "brier_score": float(np.mean((probabilities.astype(np.float64) - labels) ** 2)),
        "ece_global": float(ece_global),
        "ece_foreground": float(ece_foreground),
        "n_images": int(targets.shape[0]),
        "n_pixels": int(targets.size),
        **counts,
    })
    if include_boundary:
