...
def plot_threshold_metric(rows, metric, output_dir):
    variants = sorted({row['variant'] for row in rows})
    plt.figure(figsize=(6, 4))
    for variant in variants:
        variant_rows = sorted((row for row in rows if row['variant'] == variant), key=lambda row: float(row['threshold']))
...
def plot_reliability(cache, temperature, ece_bins, output_dir):
    variants = {'uncalibrated': 1.0, 'temperature_scaled': temperature}
    plt.figure(figsize=(5, 5))
    plt.plot([0, 1], [0, 1], linestyle='--', color='black', linewidth=1, label='perfect')
    for variant, temp in variants.items():
        rows = reliability_rows(cache['logits'], cache['targets'], temperature=temp, ece_bins=ece_bins)
        xs = []
        ys = []
        for row in rows:
...
def plot_all(*, sweep_path, val_cache_path, temperature_path, output_dir, ece_bins):
    output_dir.mkdir(parents=True, exist_ok=True)
    rows = read_sweep(sweep_path)
    paths = [plot_threshold_metric(rows, 'dice', output_dir), plot_threshold_metric(rows, 'recall', output_dir), plot_threshold_metric(rows, 'precision', output_dir)]
