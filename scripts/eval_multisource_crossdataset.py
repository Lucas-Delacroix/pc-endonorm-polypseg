...
import cv2
import torch
from data.datasets.multisource import MultiSourceSegmentationDataset, build_source_dataset, normalized_name
...
REQUIRED_EXPERIMENTS = ('A_baseline_rgb', 'B2_clahe_replace', 'F_pc_endonorm', 'G_aug_jitter', 'I_aug_combo', 'K_robust_full', 'K3_robust_balanced_tversky', 'C1_consistency', 'FD1_dino_distill', 'MS_baseline_rgb', 'C2_consistency_multisource', 'FD2_dino_distill_multisource')
REQUIRED_DATASETS = ('Kvasir', 'CVC-ColonDB', 'ETIS-Larib')
OUTPUT_DATASET_KEYS = {'Kvasir': 'kvasir', 'CVC-ColonDB': 'cvc_colondb', 'ETIS-Larib': 'etis_larib', 'CVC-ClinicDB': 'cvc_clinicdb'}
DATASET_LABELS = {'kvasir': 'Kvasir', 'kvasir_val': 'Kvasir', 'kvasir_test': 'Kvasir', 'cvc_colondb': 'CVC-ColonDB', 'cvc-colondb': 'CVC-ColonDB', 'etis_larib': 'ETIS-Larib', 'etis-larib': 'ETIS-Larib', 'cvc_clinicdb': 'CVC-ClinicDB', 'cvc_clinicdb_val': 'CVC-ClinicDB'}
EXPERIMENT_ALIASES = {'esfpnet': 'A_baseline_rgb', 'ESFPNet': 'A_baseline_rgb'}
METRIC_FIELDS = ('dice', 'iou', 'precision', 'recall', 'hd95', 'assd', 'boundary_f1')

def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate MS_baseline_rgb on each configured cross-dataset test source.')
...
def checkpoint_path(config, explicit):
    if explicit:
...
def build_eval_datasets(config):
    ...
    for source in sources:
        try:
            dataset, split_description = build_source_dataset(source, transform=transform, split_config=split_config, base_dir=config['dataset'].get('base_dir'))
...
def build_train_dataset_for_leakage(config):
    split_config = config['dataset'].get('split', {})
    datasets = []
    for source in source_list(config['dataset']['train']):
        dataset, _ = build_source_dataset(source, transform=None, split_config=split_config, base_dir=config['dataset'].get('base_dir'))
        datasets.append(dataset)
    return MultiSourceSegmentationDataset(datasets)

@torch.no_grad()
def evaluate_dataset(model, dataset, device, threshold, max_samples, run_name, dataset_name):
    ...
    for index in range(limit):
        ...
        prediction = cv2.resize(probability, (target.shape[1], target.shape[0]), interpolation=cv2.INTER_LINEAR) > threshold
    ...
    return {'model': 'ESFPNet', 'train_protocol': run_name, 'test_dataset': dataset_label(dataset_name), **{key: totals[key] / n_evaluated for key in METRIC_FIELDS}, 'n_evaluated': n_evaluated, 'n_missing': n_missing}

def write_csv(rows, path):
    path.parent.mkdir(parents=True, exist_ok=True)
    fieldnames = ['model', 'train_protocol', 'test_dataset', 'dice', 'iou', 'precision', 'recall', 'hd95', 'assd', 'boundary_f1', 'n_evaluated', 'n_missing']
    with open(path, 'w', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writeheader()
        for row in rows:
            writer.writerow({**row, **{key: f'{float(row[key]):.6f}' for key in METRIC_FIELDS}})
...
def read_metric_rows(csv_path):
    ...
    with open(csv_path) as file:
        for row in csv.DictReader(file):
            ...
            rows.append({'train_protocol': protocol, 'test_dataset': row.get('test_dataset') or dataset or '', **{metric: row.get(metric, '') for metric in METRIC_FIELDS}, 'n_evaluated': row.get('n_evaluated', ''), 'n_missing': row.get('n_missing', '')})
    return rows

def write_master_comparison(tables_dir):
    candidates = [*tables_dir.glob('*crossdataset_summary.csv'), *tables_dir.glob('table2*.csv'), *Path('resultados').glob('table2*.csv')]
    by_key = {}
    ...
    for protocol in REQUIRED_EXPERIMENTS:
        for dataset in REQUIRED_DATASETS:
            ...
            if row is None:
                row = {'train_protocol': protocol, 'test_dataset': dataset, **{metric: '' for metric in METRIC_FIELDS}, 'n_evaluated': '', 'n_missing': ''}
            rows.append(row)
...
def main():
    ...
    for dataset_name, dataset in datasets.items():
        row = evaluate_dataset(model, dataset, device, args.threshold, max_samples, run_name, dataset_name)
        rows.append(row)
