...
def default_checkpoint_path(config):
...
def original_size(image_path):
    image = cv2.imread(image_path)
    if image is None:
        raise ValueError(f'Invalid image: {image_path}')
    return (image.shape[0], image.shape[1])

def save_prediction(probability, image_path, output_dir):
...
@torch.no_grad()
def export(model, dataset, device, output_dir):
    model.eval()
...
def main():
    ...
    checkpoint_path = Path(args.checkpoint) if args.checkpoint else default_checkpoint_path(config)
    if not checkpoint_path.exists():
        raise FileNotFoundError(f'Checkpoint not found: {checkpoint_path}. Run training first (make train).')
    ...
    dataset = dataset_cls(root=args.data_root, split=args.split, transform=get_val_transforms(image_size), image_size=image_size, input_mode=config['data'].get('input_mode', 'rgb'), preprocessed_root=args.preprocessed_root or config['data'].get('preprocessed_root'))
