...
from training.configured_datamodule import build_datamodule, checkpoint_dir_for, is_multisource_config, save_resolved_config, smoke_check_batch, write_train_leakage_report

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--config', type=str, default='configs/models/esfpnet.yaml')
    parser.add_argument('--data_root', type=str, default='data/raw/kvasir-seg')
    parser.add_argument('--epochs', type=int, default=None, help='Overrides config value when set')
    parser.add_argument('--smoke_test', action='store_true', help='Runs 3 epochs only to confirm the pipeline works')
...
def resolve_default_path(config_path, default):
...
def main():
    ...
    if args.smoke_test:
        ...
        config['training']['batch_size'] = min(config['training'].get('batch_size', 8), 2)
        config['data']['image_size'] = min(config['data'].get('image_size', 352), 128)
    ...
    if args.smoke_test:
        ...
        if hasattr(dm, '_test_datasets'):
            dm._test_datasets = {name: truncate_dataset(dataset, 4) for name, dataset in dm._test_datasets.items()}
            for name, dataset in dm._test_datasets.items():
                print(f'  Test {name}: {len(dataset)} images')
        smoke_check_batch(dm.train_loader())
    print('\nBuilding model...')
    model = get_model(model_config['name'], num_classes=model_config['num_classes'], model_type=model_config.get('model_type', 'b2'), in_channels=model_config.get('in_channels', 3), pretrained_path=model_config.get('pretrained_path'))
    total_params = sum((p.numel() for p in model.parameters())) / 1000000.0
    print(f"Model: {model_config['name']} — {total_params:.1f}M parameters")
    print('\nStarting training...')
    trainer = Trainer(model=model, train_loader=dm.train_loader(), val_loader=dm.val_loader(), config=training_config)
    history = trainer.fit()
    ...
    with open(csv_path, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['epoch', 'train_loss', 'train_dice', 'val_loss', 'val_dice', 'val_iou', 'val_precision', 'val_recall', 'lr'])
        writer.writeheader()
        for record in history:
            writer.writerow({'epoch': record['epoch'], 'train_loss': round(record['train']['loss'], 6), 'train_dice': round(record['train']['dice'], 6), 'val_loss': round(record['val']['loss'], 6), 'val_dice': round(record['val']['dice'], 6), 'val_iou': round(record['val']['iou'], 6), 'val_precision': round(record['val']['precision'], 6), 'val_recall': round(record['val']['recall'], 6), 'lr': record['lr']})
