...
def parse_args():
    ...
    parser.add_argument('--model_type', default='b2', choices=sorted(MIT_DEPTHS.keys()), help='MiT variant to download.')
    parser.add_argument('--output', type=str, default=None, help='Output .pth path. Defaults to weights/mit_<model_type>.pth.')
    parser.add_argument('--url', type=str, default=None, help='Override download URL.')
    parser.add_argument('--checkpoint_format', choices=('hf', 'mit'), default='hf', help="Use 'hf' for Hugging Face SegFormer weights or 'mit' for native MiT keys.")
...
def convert_hf_segformer_to_mit(state_dict, model_type):
    ...
    def concat(source_a, source_b, target):
    ...
    for stage_idx, depth in enumerate(depths, start=1):
        ...
        for block_idx in range(depth):
            ...
            concat(f'{hf_block}.attention.self.key.weight', f'{hf_block}.attention.self.value.weight', f'{mit_block}.attn.kv.weight')
            concat(f'{hf_block}.attention.self.key.bias', f'{hf_block}.attention.self.value.bias', f'{mit_block}.attn.kv.bias')
