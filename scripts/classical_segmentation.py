...
from segmentation.classical import METHODS, segment
try:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Classical (training-free) PDI polyp segmentation as a comparison baseline."
    )
    parser.add_argument("--data-root", default=DEFAULT_DATA_ROOT)
    parser.add_argument("--split-file", default=DEFAULT_SPLIT_FILE)
    parser.add_argument("--split", default="test", choices=("train", "val", "test"))
    parser.add_argument("--external", action="store_true")
    parser.add_argument("--method", default="otsu", choices=list(METHODS))
...
def main() -> None:
    args = parse_args()
    name = args.name or f"Classical_{args.method.capitalize()}"
    ...
    for image_path, _ in samples:
        ...
        mask = segment(bgr, method=args.method)
