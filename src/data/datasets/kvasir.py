...
MODE_COMPONENTS = {'rgb': ('rgb',), 'clahe': ('clahe',), 'rgb_norm_phase_morph': ('rgb', 'norm', 'phase', 'morph')}
IMAGENET_MEAN = (0.485, 0.456, 0.406)
IMAGENET_STD = (0.229, 0.224, 0.225)
MAP_MEAN = (0.5,)
MAP_STD = (0.5,)
COMPONENT_STATS = {'rgb': (IMAGENET_MEAN, IMAGENET_STD), 'norm': (IMAGENET_MEAN, IMAGENET_STD), 'clahe': (IMAGENET_MEAN, IMAGENET_STD), 'phase': (MAP_MEAN, MAP_STD), 'morph': (MAP_MEAN, MAP_STD)}
...
class KvasirDataset(BaseDataset):
    ...
    def _load_samples(self):
        ...
        if missing_masks:
            ...
            raise FileNotFoundError(f'Missing {len(missing_masks)} masks for Kvasir images. Examples: {examples}')
        split_indices = self._get_or_create_split(len(all_images))
        indices = split_indices[self.split]
        if not indices:
            raise ValueError(f"Split '{self.split}' is empty in {self.SPLIT_FILE}. Remove the split file and run again.")
        return [(str(all_images[i]), str(all_masks[i])) for i in indices]

    def _get_or_create_split(self, total):
        ...
        n_val = int(total * 0.1)
        splits = {'dataset': 'kvasir', 'total': total, 'seed': self.SPLIT_SEED, 'train': indices[:n_train], 'val': indices[n_train:n_train + n_val], 'test': indices[n_train + n_val:]}
    ...
    def _multichannel_item(self, image_path, mask_path):
        ...
        return {'image': torch.from_numpy(stacked.transpose(2, 0, 1).copy()).float(), 'mask': torch.from_numpy(out['mask']).unsqueeze(0).float(), 'image_path': image_path}
