...
class FourierAmplitudeRandomization(A.ImageOnlyTransform):
    ...
    def _lowpass_weight(self, height, width):
...
def is_on(config, key, default=False):
    value = config.get(key, default)
    if isinstance(value, dict):
        return bool(value.get('enabled', default))
    return bool(value)

def strong_style_transforms():
    return [A.ColorJitter(**JITTER), A.RandomGamma(gamma_limit=GAMMA_LIMIT, p=0.5), A.HueSaturationValue(**HSV_LIMITS, p=0.5), A.GaussianBlur(blur_limit=BLUR_LIMIT, p=0.2), A.GaussNoise(p=0.2)]

def fourier_transform():
    return FourierAmplitudeRandomization(**FOURIER)

def build_geometric_transforms(image_size):
    return A.Compose([A.Resize(image_size, image_size), A.HorizontalFlip(p=0.5), A.VerticalFlip(p=0.5), A.Affine(translate_percent=SHIFT, scale=SCALE, rotate=ROTATE, p=0.5), A.Perspective(scale=PERSPECTIVE_SCALE, p=0.5)])

def build_appearance_transforms(augmentation):
...
def build_normalize_transforms():
    return A.Compose([A.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD), ToTensorV2()])

def build_robust_train_transforms(image_size, augmentation):
    return A.Compose([*build_geometric_transforms(image_size).transforms, *build_appearance_transforms(augmentation).transforms, *build_normalize_transforms().transforms])
