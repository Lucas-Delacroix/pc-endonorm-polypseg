...
class MultiSourcePolypDataModule:
    ...
    def __init__(self, dataset_config, image_size=352, batch_size=8, num_workers=4, pin_memory=True, augmentation=None, base_dir=None, consistency=None):
    ...
    def _sources_from_section(self, section):
    ...
    def _validate_sources(self, train_sources, test_sources):
        for source in train_sources:
            name = normalized_name(source.get('name', ''))
            if any((forbidden in name for forbidden in EXTERNAL_TEST_ONLY)):
                raise ValueError(f"Source '{source.get('name')}' is test-only and cannot be used for training.")
    ...
    def _build_multi_dataset(self, sources, transform):
        split_config = self.dataset_config.get('split', {})
        datasets = []
        for source in sources:
            dataset, split_description = build_source_dataset(source, transform=transform, split_config=split_config, base_dir=self.base_dir)
    ...
    def setup(self):
        ...
        self._test_datasets = {source['name']: self._build_multi_dataset([source], get_val_transforms(self.image_size)) for source in test_sources}
        self._log_split_info()

    def _log_counts(self, label, dataset):
    ...
    def test_loaders(self):
        return {name: DataLoader(dataset, batch_size=1, shuffle=False, num_workers=self.num_workers, pin_memory=self.pin_memory) for name, dataset in self._test_datasets.items()}
