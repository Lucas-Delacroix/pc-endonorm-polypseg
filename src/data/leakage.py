...
def image_index(samples):
    return {normalized_name(image_path): image_path for image_path, _ in samples}

def write_leakage_report(train_dataset, test_datasets, output_path, train_protocol):
    ...
    for test_name, test_dataset in test_datasets.items():
        ...
        if overlap:
            for name in overlap:
                rows.append({'train_protocol': train_protocol, 'test_dataset': test_name, 'n_train': len(train_samples), 'n_test': len(test_samples), 'n_overlap': len(overlap), 'normalized_file': name, 'train_image_path': train_by_name[name], 'test_image_path': test_by_name[name], 'status': 'leak'})
        else:
            rows.append({'train_protocol': train_protocol, 'test_dataset': test_name, 'n_train': len(train_samples), 'n_test': len(test_samples), 'n_overlap': 0, 'normalized_file': '', 'train_image_path': '', 'test_image_path': '', 'status': 'ok'})
    fieldnames = ['train_protocol', 'test_dataset', 'n_train', 'n_test', 'n_overlap', 'normalized_file', 'train_image_path', 'test_image_path', 'status']
